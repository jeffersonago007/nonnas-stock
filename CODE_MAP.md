...
Estado: pós-T20 + T-LOT-09 (MVP 1.0 em produção interna) + T-CANAL-00..06 (MVP 1.2 canais completo — fundação + adapter genérico + pipeline de baixa de estoque + CRUD APIs + UI + scheduler ativo de processamento + discriminação de taxas no detalhe do pedido).
...
| **Como funciona o cálculo de CMV?** | T-CMV-01: cada entrada em insumo AGREGADOR (`controlaValidade=false`) chama [`BuscarOuCriarLoteAgregadorUseCase.executeComCusto`](inventory-core/src/main/java/com/nonnas/inventory/application/movimentacao/BuscarOuCriarLoteAgregadorUseCase.java) que recalcula custo médio ponderado: `(saldo_anterior × custo_anterior + qtd_entrada × custo_entrada) / (saldo_anterior + qtd_entrada)` cross-filial. Atualiza `Lote.valorUnitario` no agregador. RASTREADO mantém custo histórico fixo (FIFO real). Saídas (`RegistrarSaidaMultiItemUseCase`) já gravam `items_movimentacao.valor_unitario = lote.valorUnitario` no momento da venda — snapshot histórico preservado. CMV agregado em 3 SQLs nativos cross-context em [CmvQueriesJdbc](reporting/src/main/java/com/nonnas/reporting/infrastructure/persistence/CmvQueriesJdbc.java): por insumo (direto), por produto vendável (COALESCE FICHA_TECNICA → produto OU PRODUTO_REVENDA direto), por canal (JOIN com `pedidos_canais.movimentacao_id` + valor líquido T-CANAL-06 = margem bruta). UI: aba CMV em `/relatorios` com 3 sub-abas. |
