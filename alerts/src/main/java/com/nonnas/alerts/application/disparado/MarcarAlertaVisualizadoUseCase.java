...
@Service
public class MarcarAlertaVisualizadoUseCase {

    private final AlertaDisparadoRepository repo;
    private final Clock clock;

    public MarcarAlertaVisualizadoUseCase(AlertaDisparadoRepository repo, Clock clock) {
        this.repo = repo;
        this.clock = clock;
    }

    @Transactional
    public AlertaDisparado execute(UUID alertaId, UUID visualizadoPor) {
        AlertaDisparado a = repo.findById(AlertaDisparadoId.of(alertaId))
                .orElseThrow(() -> new NotFoundException("Alerta disparado", alertaId));
        a.marcarVisualizado(visualizadoPor, clock.instant());
        return repo.save(a);
    }
}
