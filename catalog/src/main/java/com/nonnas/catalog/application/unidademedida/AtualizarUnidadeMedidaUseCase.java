...
@Service
public class AtualizarUnidadeMedidaUseCase {

    private final UnidadeMedidaRepository repository;
    private final Clock clock;

    public AtualizarUnidadeMedidaUseCase(UnidadeMedidaRepository repository, Clock clock) {
        this.repository = repository;
        this.clock = clock;
    }

    @Transactional
    public UnidadeMedida execute(UUID id, String novoNome) {
        UnidadeMedida unidade = repository.findById(UnidadeMedidaId.of(id))
                .orElseThrow(() -> new NotFoundException("UnidadeMedida", id));
        unidade.renomear(novoNome, clock.instant());
