...
public class AtualizarUsuarioUseCase {
    ...
    public AtualizarUsuarioUseCase(UsuarioRepository repository, Clock clock) {
        this.repository = repository;
        this.clock = clock;
    }

    @Transactional
    public Usuario execute(UUID id, String novoNome) {
        Usuario usuario = repository.findById(UsuarioId.of(id))
                .orElseThrow(() -> new NotFoundException("Usuario", id));
        usuario.renomear(novoNome, clock.instant());
